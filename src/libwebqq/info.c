...
void lwqq_info_get_long_nick(LwqqClient* lc, LwqqBuddy* buddy)
{
    //{"retcode":0,"result":[{"uin":1503539798,"lnick":"有梦想的人是幸福的，哪怕是梦想没有实现，只要努力奋斗了，也无怨无悔"}]}
    if(!lc||!buddy||!buddy->uin) 
        return ;

    char url[512];
	json_t *json = NULL, *json_tmp;

    snprintf(url,sizeof(url),"http://s.web2.qq.com/api/get_single_long_nick2?tuin=%s&vfwebqq=%s&t=%ld", buddy->uin,lc->vfwebqq,time(NULL));
    ...
    char *cookies = lwqq_get_cookies(lc);
    if (cookies) {
    ...
    if(json_tmp->child)
    {
        ...
        buddy->long_nick = s_strdup(json_parse_simple_value(json, "lnick"));
    }
