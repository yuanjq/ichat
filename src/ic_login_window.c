...
GtkWidget *ic_login_window_new() {
	...
	strcpy(data,	"GtkWindow {" 
					"border-radius: 5px;"
					"}"
					"#login_window {background-image: url('");
	strcat(data, RESDIR);
	strcat(data, "images/background.png');}");
	*/
...
void conn_timeout_error_dialog_show(GtkWidget *window) {
	GtkWidget *dialog;
	dialog = gtk_message_dialog_new(GTK_WINDOW(window), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_WARNING, GTK_BUTTONS_CLOSE, "网络连接超时，请检查网络连接!");
	gtk_dialog_run(GTK_DIALOG(dialog));
	gtk_widget_destroy(dialog);
}

void user_pwd_error_dialog_show(GtkWidget *window) {
	GtkWidget *dialog;
	dialog = gtk_message_dialog_new(GTK_WINDOW(window), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_WARNING, GTK_BUTTONS_CLOSE, "用户名或密码错误，请重新输入!");
	gtk_dialog_run(GTK_DIALOG(dialog));
	gtk_widget_destroy(dialog);
}
